"""
Report generator module for Well-Architected Tool Updater

This module collects compliance data from AWS Config Conformance Packs
...
def generate_and_upload_report(config_client, conformance_packs, pillar_mapping, bucket_name, html_generator, s3_uploader):
    """
    Generate HTML report and upload to S3
    
    Args:
        config_client: Boto3 AWS Config client
        conformance_packs: List of conformance pack names
        pillar_mapping: Dictionary mapping conformance pack names to pillar IDs
        bucket_name: S3 bucket name for report upload
        html_generator: Function to generate HTML from compliance data
        s3_uploader: Function to upload HTML to S3
        
    Returns:
        S3 URL of the uploaded report or empty string if failed
    """
    try:
        ...
        compliance_data = collect_compliance_data(config_client, conformance_packs, pillar_mapping)
        
        # Generate HTML report
        html_content = html_generator(compliance_data, account_id, region)
        
        # Upload to S3
        return s3_uploader(html_content, bucket_name)
    except Exception as e:
        logger.error(f"Error generating and uploading report: {e}")
        return ""
